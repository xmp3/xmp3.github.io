...
export default {
  ...
  timeUpdate() {
    ...
    const formattedTime = u.formatTime(currentTime)
  ...
  update() {
    ...
    document.title = u.title(this.currentAudio.title)
    window.location.hash = this.currentPlaying + 1

    this.cover.src = u.imageURL(this.currentAudio.cover)
    ...
    const audio = u.audioURL(this.currentAudio.file)
    elements.createAudioElement.call(this, audio)

    this.downloadButton.href = audio
    this.downloadButton.download = u.download(
