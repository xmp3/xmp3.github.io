...
module Jekyll
  module GitHub
    ...
    def self.get_collections()
      ...
      GitHub.fetch_csv(repo_config, username, repo_config['filename']) do |row|
        {
    ...
    def self.get_tracks(username, filename)
      ...
      repo_config = config['github']['tracks']

      path = "#{username}/#{filename}"
      GitHub.fetch_csv(repo_config, github_username, path) do |row|
        {
          'title' => row['title'],
          'artist' => row['artist'],
          'cover' => row['cover'],
